...
object LineTools {
    ...
    fun extractText(str: String): String = if (isTodo(str)) str.substring(TODO_DONE.length) else str
    private fun createLine(str: String): Line = if(isTodo(str)) Line(extractText(str), isTodoDone(str)) else Line(text=str)
    fun stringToLines(str: String): ArrayList<Line> = ArrayList(str.split(SEP).toList().map { createLine(it)})
